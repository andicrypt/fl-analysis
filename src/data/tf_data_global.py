...
class GlobalDataset:
    ...
    def get_test_batch(self, batch_size, max_num_batches=-1):
        ...
        ds = self.test_generator.batch(batch_size)
        if max_num_batches and max_num_batches > 0:
            ds = ds.take(max_num_batches)
        ds = ds.map(image_augmentation.test_augment, num_parallel_calls=tf.data.AUTOTUNE)
    ...
    def get_aux_generator(self, batch_size, aux_size, augment_cifar, attack_type, max_test_batches):
        if aux_size == 0:
            ...
            test_dataset = tf.data.Dataset.from_tensor_slices((self.x_aux_test, self.mal_aux_labels_test)) \
                ...
                .map(image_augmentation.pixel_pattern_if_needed(attack_type == 'pixel_pattern'), num_parallel_calls=tf.data.AUTOTUNE)
        ...
        if augment_cifar:
            return test_dataset\
                .map(image_augmentation.test_aux_augment, num_parallel_calls=tf.data.AUTOTUNE) \
                .prefetch(tf.data.AUTOTUNE)
        else:
